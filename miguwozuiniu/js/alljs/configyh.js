var channel = "014DB03",//渠道号
 ...
 //随机六位数
	...
	//data加密获取
	function encrypt(channel,version,client,device,os,mac){
	 jsonData={"channel":"014DB03","version":"1.0","client":"linux_box","device":device,"os":"iOS9.3","mac":mac};
		return DES3.encrypt(desMa,JSON.stringify(jsonData));
	}
 
	 //token加密获取
	function token(service,time,data,salt,version,secureKey){
	  	return $.md5(service+time+encrypt(channel,version,client,device,os,mac)+salt+version+secureKey);
	  }
 ...
 //数据请求
 requestAjax("http://172.16.4.196:8080/game_music/do/jsonp","post",data,function(res){
 	var	objSalt3des = $.md5(res.salt+secureKey);
 	var bodyobj = DES3.decrypt(objSalt3des,res.body);	
